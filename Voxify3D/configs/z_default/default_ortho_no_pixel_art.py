...
'''
coarse_train = dict(
    N_iters=15000, #8000,                # number of optimization steps   # default 5000
    N_rand=8192,                  # batch size (number of random rays per optimization step)
    lrate_density=1e-1,           # lr of density voxel grid
    lrate_k0=1e-1,                # lr of color/feature voxel grid
    lrate_uncertainty = 1e-2,      # Uncertainty
    ...
    weight_entropy_uncertainty  = 0.001,    # Uncertainty
    weight_nearclip=0,
    weight_distortion=0,
    ...
    weight_tv_uncertainty=0.0,             # weight of total variation loss of color/feature voxel grid
...
coarse_model_and_render = dict(
    ...
    alpha_init=1e-6,              # set the alpha values everywhere at the begin of training # origin 1e-4
...
fine_model_and_render.update(dict(
    num_voxels=num_voxels_ini*100,          #3072000 -> 3 voxel...   #10240000 -> 9....       # expected number of voxel #1024000  # 1024只有幾塊。。
    num_voxels_base=num_voxels_ini,
    rgbnet_dim=12, 
    alpha_init=1e-2,
