...
std::vector<torch::Tensor> alpha2weightkk_cuda(torch::Tensor alpha, torch::Tensor ray_id, const int n_rays);
// std::vector<torch::Tensor> alpha2weightkk_cuda(torch::Tensor alpha, torch::Tensor ray_id, const int n_rays, torch::Tensor alpha_std, torch::Tensor alpha_std_sum);
...
std::vector<torch::Tensor> alpha2weightkk(torch::Tensor alpha, torch::Tensor ray_id, const int n_rays) {
// std::vector<torch::Tensor> alpha2weightkk(torch::Tensor alpha, torch::Tensor ray_id, const int n_rays, torch::Tensor alpha_std, torch::Tensor alpha_std_sum) {
  CHECK_INPUT(alpha);
  CHECK_INPUT(ray_id);
  assert(alpha.dim()==2);
  assert(ray_id.dim()==1);
  return alpha2weightkk_cuda(alpha, ray_id, n_rays);
  // return alpha2weightkk_cuda(alpha, ray_id, n_rays, alpha_std, alpha_std_sum);
}
...
torch::Tensor alpha2weightkk_backward(
        torch::Tensor alpha, torch::Tensor weight, torch::Tensor T, torch::Tensor alphainv_last,
        torch::Tensor i_start, torch::Tensor i_end, torch::Tensor i_end_k, const int n_rays,
        torch::Tensor grad_weights, torch::Tensor grad_last) {
  CHECK_INPUT(alpha);
  ...
  CHECK_INPUT(grad_weights);
  CHECK_INPUT(grad_last);
  return alpha2weightkk_backward_cuda(
          alpha, weight, T, alphainv_last,
          i_start, i_end, i_end_k, n_rays,
          grad_weights, grad_last);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("alpha2weightkk", &alpha2weightkk, "Per-point alpha to accumulated blending weight for k samples");
  m.def("alpha2weight_backward", &alpha2weight_backward, "Backward pass of alpha2weight");
  m.def("alpha2weightkk_backward", &alpha2weightkk_backward, "Backward pass of alpha2weight");
}
