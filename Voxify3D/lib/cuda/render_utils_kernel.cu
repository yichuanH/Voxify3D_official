 ...
 */
template <typename scalar_t>
__global__ void alpha2weightkk_cuda_kernel(
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> alpha_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> weight_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> T_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> alphainv_last_k,
    const int n_rays,
    const int k_samples,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> i_end_k,
    int64_t* __restrict__ i_start,
    int64_t* __restrict__ i_end) {
  ...
  if(i_ray<n_rays) {
    ...
    int i,j,k;
    for(k=0; k<k_samples; ++k) {
      float T_cum = 1.;
      float flag = 0;
      for(i=i_s; i<i_e_max; ++i) {
        T_k[i][k] = T_cum;
        weight_k[i][k] = T_cum * alpha_k[i][k];
        T_cum *= (1. - alpha_k[i][k]);
        if(T_cum<1e-3) {
          i+=1;
          for (j=i; j<i_e_max; ++j) {
      ...
      i_end_k[i_ray][k] = i;
      alphainv_last_k[i_ray][k] = T_cum;
    }
  }
}

__global__ void __set_i_for_segment_start_endkk(
        int64_t* __restrict__ ray_id,
        const int n_pts,
        int64_t* __restrict__ i_start,
        int64_t* __restrict__ i_end) {
...
std::vector<torch::Tensor> alpha2weightkk_cuda(torch::Tensor alpha_k, torch::Tensor ray_id, const int n_rays) {
// std::vector<torch::Tensor> alpha2weightkk_cuda(torch::Tensor alpha_k, torch::Tensor ray_id, const int n_rays, torch::Tensor alpha_std, torch::Tensor alpha_std_sum) {

  const int n_pts = alpha_k.size(0);
  const int k_samples = alpha_k.size(1);
  const int threads = 256;

  auto weight_k = torch::zeros_like(alpha_k);
  auto T_k = torch::ones_like(alpha_k);
  auto alphainv_last_k = torch::ones({n_rays, k_samples}, alpha_k.options());
  auto i_start = torch::zeros({n_rays}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  auto i_end = torch::zeros({n_rays}, torch::dtype(torch::kInt64).device(torch::kCUDA));
  auto i_end_k = torch::zeros({n_rays, k_samples}, alpha_k.options());
  if(n_pts==0) {
    return {weight_k, T_k, alphainv_last_k, i_start, i_end, i_end_k};
  }

  __set_i_for_segment_start_endkk<<<(n_pts+threads-1)/threads, threads>>>(
          ray_id.data<int64_t>(), n_pts, i_start.data<int64_t>(), i_end.data<int64_t>());
  i_end[ray_id[n_pts-1]] = n_pts;

  const int blocks = (n_rays + threads - 1) / threads;

  AT_DISPATCH_FLOATING_TYPES(alpha_k.type(), "alpha2weightkk_cuda", ([&] {
    alpha2weightkk_cuda_kernel<scalar_t><<<blocks, threads>>>(
        alpha_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        weight_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        T_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        alphainv_last_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        n_rays,
        k_samples,
        i_end_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        i_start.data<int64_t>(),
        i_end.data<int64_t>());
  }));
...
template <typename scalar_t>
__global__ void alpha2weightkk_backward_cuda_kernel(
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> alpha_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> weight_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> T_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> alphainv_last_k,
    ...
    const int n_rays,
    const int k_samples,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> grad_weights_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> grad_last_k,
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> grad_k) {

  const int i_ray = blockIdx.x * blockDim.x + threadIdx.x;
  if(i_ray<n_rays) {
    for(int k=0; k<k_samples; ++k) {
      const int i_s = i_start[i_ray];
      const int i_e = i_end_k[i_ray][k];
      float back_cum = grad_last_k[i_ray][k] * alphainv_last_k[i_ray][k];
      for(int i=i_e-1; i>=i_s; --i) {
        grad_k[i][k] = grad_weights_k[i][k] * T_k[i][k] - back_cum / (1-alpha_k[i][k] + 1e-10);
        back_cum += grad_weights_k[i][k] * weight_k[i][k];
      }
    }
  }
}

torch::Tensor alpha2weightkk_backward_cuda(
        torch::Tensor alpha_k, torch::Tensor weight_k, torch::Tensor T_k, torch::Tensor alphainv_last_k,
        torch::Tensor i_start, torch::Tensor i_end, torch::Tensor i_end_k, const int n_rays,
        torch::Tensor grad_weights_k, torch::Tensor grad_last_k) {

  auto grad_k = torch::zeros_like(alpha_k);
  if(n_rays==0) {
    return grad_k;
  }
  ...
  AT_DISPATCH_FLOATING_TYPES(alpha_k.type(), "alpha2weightkk_backward_cuda", ([&] {
    alpha2weightkk_backward_cuda_kernel<scalar_t><<<blocks, threads>>>(
        alpha_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        weight_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        T_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        alphainv_last_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        ...
        n_rays,
        k_samples,
        grad_weights_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        grad_last_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
        grad_k.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>());
  }));
