    ...
    def voxel_count_views(self, rays_o_tr, rays_d_tr, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        ...
        for rays_o_, rays_d_ in zip(rays_o_tr.split(imsz), rays_d_tr.split(imsz)):
            ...
            for rays_o, rays_d in zip(rays_o_, rays_d_):
                vec = torch.where(rays_d==0, torch.full_like(rays_d, 1e-6), rays_d) # max 0.8090 # min -0.6866
                rate_a = (self.xyz_max - rays_o) / vec  # !Problem # max 57 
                rate_b = (self.xyz_min - rays_o) / vec  # !Problem # max 284
                # self.xyz_max = tensor([13.1278, 13.1927, 13.2311])
                # (self.xyz_max - rays_o) max 24.4975
                # (self.xyz_max - rays_o) min 6.2392

                t_min = torch.minimum(rate_a, rate_b).amax(-1).clamp(min=near, max=far) # 2.
                t_max = torch.maximum(rate_a, rate_b).amin(-1).clamp(min=near, max=far) # 26.1651
...
def get_rays(H, W, K, c2w, inverse_y, flip_x, flip_y, mode='center'):
    ...
    # Rotate ray directions from camera frame to the world frame  # C->W
    ...
    #print("get_rays")
    #import ipdb; ipdb.set_trace()  ## break pointimport ipdb;
    #print("------ rays_o -----")
    #print(rays_o)
    #print("------ rays_d ------")
    #print(rays_d)
