...
def load_data(args):
    ...
    if args.dataset_type == 'llff':
        ...
        uncertainty_mask, depth_gt, images, poses, render_poses, hwf, i_split, images_gt = load_blender_data(args.datadir, args.half_res, args.testskip)
        ...
        if images.shape[-1] == 4:    ############ RGBA
            if args.white_bkgd:  
                images = images[...,:3]*images[...,-1:] + (1.-images[...,-1:])   ##Set White
            else:
                images = images[...,:3]*images[...,-1:]  ## Set Black 

        ### ---- imgs gt ------ ###
        if images_gt.shape[-1] == 4:    ############ RGBA
            if args.white_bkgd:  
                images_gt = images_gt[...,:3]*images_gt[...,-1:] + (1.-images_gt[...,-1:])   ##Set White
            else:
                images_gt = images_gt[...,:3]*images_gt[...,-1:]  ## Set Black 

        #images shape [18, 1200, 1200, 3]
        #print("load_data")
        #breakpoint() # check is blender or not
    ...
    data_dict = dict(
        ...
        i_train=i_train, i_val=i_val, i_all=i_all, i_test=i_test,
